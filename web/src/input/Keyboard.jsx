...
const Keyboard = ({ numberOfOctaves = 2 }) => {
  ...
  const playingKeyCodes = useMemo(
    () =>
      Object.keys(playingKeysByKeycode)
        .filter((keyCode) => playingKeysByKeycode[keyCode])
        .map((keyCode) => parseInt(keyCode)),
    [playingKeysByKeycode]
  );
    ...
    <div styleName="container" style={{ gridAutoColumns: whiteKeyWidth }}>
      {keyboardLayout.map((value, index) =>
        value ? (
          <WhiteKey
            key={index}
            isPressed={playingKeyCodes.some(
              (playingKeyCode) => keyCodeToIndex.indexOf(playingKeyCode) === index
            )}
            index={index}
          />
        ) : (
...
const BlackKey = ({ index, isPressed, keyboardLayout }) => {
  ...
  const numberOfWhiteKeysBefore = useMemo(
    () => getNumberOfWhiteKeysBefore(keyboardLayout, index),
    [index, keyboardLayout]
  );
  ...
  return (
    <div
      ...
      styleName="black-key"
      style={{
        left: `${numberOfWhiteKeysBefore * whiteKeyWidth - blackKeyWidth / 2}px`,
        width: blackKeyWidth,
        backgroundColor: isPressed ? "grey" : "black",
      }}
    >
...
const WhiteKey = ({ index, isPressed }) => {
  ...
  return (
    <div
      {...getEventHandlers()}
      styleName="white-key"
      style={{ backgroundColor: isPressed ? "lightgrey" : "white" }}
    >
      <span styleName="key-notation">{indexToKey[index] || ""}</span>
    </div>
  );
...
const getMouseEvents = (index) => ({
  onMouseDown: () => virtualKeyboardKeyDown(index),
  onMouseUp: () => virtualKeyboardKeyUp(index),
  onMouseLeave: () => virtualKeyboardKeyUp(index),
});

const getTouchEvents = (index) => ({
  onTouchStart: () => virtualKeyboardKeyDown(index),
  onTouchEnd: () => virtualKeyboardKeyUp(index),
});
...
const getNumberOfWhiteKeysBefore = (keyboardLayout, index) =>
  keyboardLayout.slice(0, index).reduce((acc, value) => acc + value, 0);
