...
const Sequencer = ({
  ...
  const changeMarkerValue = useCallback(
    (index, value) => {
    ...
    <div styleName="container">
      ...
      <div>
        <span
          ...
          onClick={() => setSequenceSize(sequenceSize === 8 ? 16 : 8)}
        >
          Beat: 1/{sequenceSize}
        </span>
        <span
          styleName={`envelope ${isEnvelopeOn ? "on" : "off"}`}
          onClick={() => setIsEnvelopeOn(!isEnvelopeOn)}
        >
          Envelope
        </span>
      </div>
...
function envelope(y, x, size) {
  ...
  return x < attack * size ? y * (x * m1) : y * (x * m2 + 1 / (1 - attack));
}
